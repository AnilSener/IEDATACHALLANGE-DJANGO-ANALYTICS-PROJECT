...
from hotelapp.models import Review,AttractionSentiment
...
df = pd.DataFrame(list(Review.objects.values_list( "id","review","title","rating")),columns=["id","review","title","rating"])
...
for i,review in enumerate(df["review"]):
    ...
    for idx,taggedsent in enumerate(taggedlist):
        ...
        for idx2,t in enumerate(taggedsent):
            ...
            rev_word=AttractionSentiment()
            cnt+=1
            rev_word.seq_no=cnt
            rev_word.reviewID=Review.objects.get(id=df['id'].loc[i])
